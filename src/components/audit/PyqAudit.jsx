...
const PyqAudit = ({ papers, onUpdateCount }) => {
  ...
  if (loading) {
    return (
      <div className="py-10 text-center text-slate-400 flex flex-col items-center">
        ...
        <p className="text-xs font-bold uppercase tracking-widest">Scanning Exam Categories...</p>
      </div>
    );
  }
  ...
  if (missing.length === 0) {
    return (
      <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-12 text-center text-emerald-600 font-bold shadow-sm">
        All papers meet the PYQ requirement (Semester + Internal)!
      </div>
    );
  }
